...
clave.mayor.dist.Mah <- which(cerioli.test.interseccion$mahdist.rw == max(cerioli.test.interseccion$mahdist.rw))
...
```{r}
cerioli.test.interseccion$mahdist
clave.mayor.dist.Mah
```

Como vemos ha etiquetado como outlier con mayor distancia de Mahalanobis a la observación 11, que ya había sido etiquetada en otras pruebas como en los outliers IQR, sin embargo en esta ocasión ha sido etiquetado como outlier debido a valores raros en múltiples columnas a la vez, aunque tampoco tenemos la garantía estadistica de que esto sea cierto debido a que no se han superado los test de normalidad necesarios para aplicar este método.
...
# 5. Resumen final: Análisis de resultados

## 5.1 Conjunto de datos

Con respecto al conjunto de datos, hemos trabajado con una modificación del dataset Wine, eliminando una de sus tres clases, manteniendo diez instancias de la clase eliminada para que actuen como outliers. Las trece variables que conforman este conjunto son numéricas y con valores bastante distintos, así que se han utilizado todas.

Se ha aplicado una normalización z-score en los casos que ha sido necesario, como por ejemplo los métodos basados en distancias.

## 5.2 Outliers en una variable

### 5.2.1 Método IQR

Hemos encontrado tanto outliers no extremos como outliers extremos.

Los outliers IQR extremos se trataban de la novena y decima observación, valores de la clase eliminada para que actuaran de outliers. Estas observaciones cuentan con un valor muy inusual en la última columna, `Proline`.

Además, la columna `Proline` también nos ha servido para encontrar como outliers no extremos a gran parte de las observaciones de la clase eliminada, aunque no todos. Estas observaciones no contaban con ningún otro valor inusual en sus otras columnas. 

También se han encontrado una alta variedad de valores anómalos en el resto de columnas, aunque hay que tener en cuenta que no todas siguen una distribución similar a la normal, luego también es probable que se detecte una gran cantidad de falsos positivos. 

Entre alguno de estos elementos destacan la observación 11 debido a sus valores muy bajos en `Ash` y `Alcalinity_of_Ash`, la observación 47 por un valor muy alto en `Magnesium` y la observación 73 por un valor muy alto tanto en `Ash` como en `Flavanoids`, aunque esta última columna, `Flavanoids`, no sigue una distribución muy similar a la normal mientras que el resto de las mencionadas sí.

### 5.2.2 Test de Hipótesis

Con respecto a los test aplicados, el test de Shapiro-Wilks rechaza que la distribución es una normal para todas las columnas a excepción de la columna `Ash`, de la que no podemos descartar que siga una distribución normal.

En cuanto a los outliers encontrados, vemos que aunque encuentre valores como outliers esta información no es de utilidad, ya que las distintas columnas no siguen una distribución normal. La única columna que no podemos descartar que siga una normal, `Ash`, no detecta como outlier el elemento más alejado de la media.


## 5.3 Outliers multivariantes

### 5.3.1 Visualización con biplot

La visualización con biplot no ha dado unos resultados muy buenos, obteniendo tan solo un 51% de porcentaje del dataset explicado, por lo que aunque nos puede ayudar a hacer un esbozo de como están distribuidos los datos, realmente no se trata de un buen gráfico.

### 5.3.2 Métodos estadísticos usando la distancia de Mahalanobis

En este dataset solo tenemos una única columna que sigue una distribución normal, luego no deberíamos aplicar este método. Aun así se ha lanzado con columnas que no siguen una distribución normal, aunque su forma sea similar, para observar la información que obtenemos.

Debido a que solo una de las columnas utilizadas sigue una distribución normal, el conjunto de columnas no sigue una distribución normal multivariante, pero de nuevo, se ha probado a ejecutar este método.

Tanto el test individual como el test de intersección han devuelto outliers.

El test individual ha marcado como outliers varios de las observaciones del método IQR, entre ellas la observación 11, 47 y 73, que fueron las más destacadas por el método IQR y los test de hipótesis debido a que sus valores eran de los más alejados de las medias de varias columnas.

El test de intersección obtiene la observación 11 como único outlier, por lo que sería interesante estudiar este caso debido a que ya se ha encontrado como outlier con el método IQR y se trata del valor más alejado de la media de dos columnas.

De nuevo estos resultados no los deberíamos tener en cuenta debido a que las columnas no siguen una distribución normal multivariante, ni una distribución normal por si mismas.

### 5.3.3 LOF

A través del gráfico de puntuaciones LOF se han tenido en cuenta nueve observaciones como outliers debido a su alto valor de puntuación LOF. Entre ellos encontramos las siguientes observaciones:

- Observación 25, con un valor muy inusual muy alto en `Alcalinity_of_ash` y valores inusuales altos en `Magnesium` y `Total_phenols`.
- Observación 73, con valores muy inusuales altos en `Ash` y `Flavanoids`, así como un valor inusual alto en `Alcalinity_of_ash`.
- Observación 110, con un valor inusual alto en `Color_intensity`, así como valores inusuales altos en `Alcohol`, `Proanthocyanins`.
- Observación 11, con valores muy inusuales bajos en `Ash` y `Alcalinity_of_ash`.
- Observación 62, con un valor muy inusual alto en `Proanthocyanins`.
- Observación 47, con un valor muy inusual alto en `Magnesium` y `Proanthocyanins`.
- Observación 21, con un valor muy inusual alto en `Magnesium`.
- Observación 30, con valores inusuales altos en `Magnesium` y `Proanthocyanins`.
- Observación 23, sin valores inusuales.


Como podemos ver, la mayoría de los outliers obtenidos por LOF obtienen un valor bastante alto en `Magnesium`, `Ash` o `Proanthocyanins`, teniendo en cuenta que estas tres columnas siguen una distribución similar a una normal (una de ellas normal), sería interesante estudiar si estas características se tratan de ciertos vinos especiales, que serían de interés, o simplemente valores erroneos a la hora de tomar la muestra.

Por otro lado podemos observar que no se han marcado como outlier ninguna instancia de las diez mantenidas para que actuaran como outliers. Esto probablemente se deba que al tener ten pocos datos, si esas diez observaciones están lo suficientemente cerca sean capaces de tener un valor LOF bajo.

### 5.3.4 Métodos basados en clustering

Hemos aplicado tanto el algoritmo k-means como medoides. Debido a que teniamos información del dataset, que solo cuenta con dos clases (siendo la tercera de outliers), hemos lanzado los algoritmos con el objetivo de buscar dos clusters y de dichos clusters hemos obtenido los diez (el número de outliers mantenidos en la modificación del dataset) valores más alejados de los centroides.


En los resultados se han encontrado algunos de las observaciones que teniamos como objetivo, (la observación 2, 9 y 10 usando k-means y la 9 en medoides) mientras que el resto de outliers son observaciones en zonas del espacio poco densas, sin embargo gran parte de las observaciones objetivo no se han encontrado. Esto se puede explicar viendo el biplot, y es que dichas observaciones están relativamente juntas, y han podido causar que los centroides se vieran modificados hacia dicha zona del espacio, marcando como outlier solo los valores más extremos y otros valores del conjunto de datos que se encontraban en zonas poco densas, como la observación 110.

Estos resultados sacan a la luz uno de los principales problemas de los métodos de clustering, el cálculo de centroides puede hacer que no se obtengan todos los outliers si se tienen demasiados por la misma zona del espacio.
